        ...
        if (reminderResp['success'] == true && reminderResp['data'] is List) {
          ...
          for (final j in serverReminders) {
            ...
            if (localReminder == null) {
              ...
              // v1.0.65: 字段级 diff 同步（同 parent 端修复）
              // 修复 v1.0.64 引入的 5 个 bug（闪退 / 时间错乱 / 追响失效 / 监督条打不开 / snoozed 标签没显示）
              ...
              if (statusChanged || snoozeChanged) {
                print('🟢 子女端: 同步变更: $serverId ${localReminder.status}→$serverStatus, snooze=$serverSnoozeCount');
              }
            }
          }
        }
